...
C_DX9_CHECK_BOX::C_DX9_CHECK_BOX( C_TINY_XML_ELEMENT * pElement )
{
	SetObject( pElement );
...
BOOL C_DX9_CHECK_BOX::GetChecked()
{
	return m_bChecked;
}
void C_DX9_CHECK_BOX::SetChecked( BOOL bChecked )
{
	m_bChecked = bChecked;
}

void C_DX9_CHECK_BOX::Display()
{
...
void C_DX9_CHECK_BOX::Calculate()
{
	GetString( TRUE );
}

void C_DX9_CHECK_BOX::MouseMove(  )
{
	C_DX9_WINDOW *pWindow = (C_DX9_WINDOW *)GetParent();
	dk::DPOINT Pos = *pWindow->GetAbsolutePos() + *GetRelPos();
...
void C_DX9_CHECK_BOX::KeyEvent( _KEY sKey )
{
	if( !sKey.cKey )
	{
...
void C_DX9_CHECK_BOX::UpdateObject( int iIndex )
{

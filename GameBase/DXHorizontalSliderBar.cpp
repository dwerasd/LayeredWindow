...
C_DX9_HORIZONTAL_SLIDER_BAR::C_DX9_HORIZONTAL_SLIDER_BAR()
{
	SetDragged( FALSE );
	SetCallback( 0 );
...
C_DX9_HORIZONTAL_SLIDER_BAR::C_DX9_HORIZONTAL_SLIDER_BAR( C_TINY_XML_ELEMENT * pElement )
{
	SetDragged( FALSE );
	SetCallback( 0 );
	m_iMinValue = 0, m_iMaxValue = 0, m_iValue = 0;
...
void C_DX9_HORIZONTAL_SLIDER_BAR::SetSliderElement( C_TINY_XML_ELEMENT * pElement )
{
	SetObject( pElement );
	...
	if( pUpdater )
	{
		if( pUpdater->GetText() )
		{
			C_DX9_VARIANT * pCvar = DXENGINE()->Cvars[ pUpdater->GetText() ];
...
void C_DX9_HORIZONTAL_SLIDER_BAR::Display()
{
	C_DX9_WINDOW *pWindow = (C_DX9_WINDOW *)GetParent();
	dk::DPOINT Pos = *pWindow->GetAbsolutePos() + *GetRelPos();
	...
	if( pState )
	{
		...
		pSlider->Display( dk::DPOINT( Pos.GetX() + static_cast<int>( floor( static_cast<float>( GetWidth() ) / ( GetMaxValue() - GetMinValue() ) * ( GetValue() - GetMinValue() ) ) ) - 5, Pos.GetY() + 2 ), 10, HORIZONTAL_SLIDER_HEIGHT - 4 );

		DXENGINE()->GetFont()->DrawString( Pos.GetX() + GetWidth() / 2, Pos.GetY() - 15, FT_CENTER, pString->GetD3DCOLOR(), GetFormatted() );
	}
}

void C_DX9_HORIZONTAL_SLIDER_BAR::Calculate()
{
...
void C_DX9_HORIZONTAL_SLIDER_BAR::MouseMove(  )
{
	C_DX9_WINDOW *pWindow = (C_DX9_WINDOW *)GetParent();
	dk::DPOINT Pos = *pWindow->GetAbsolutePos() + *GetRelPos();
	...
	if( GetDragged() )
	{
		dk::DPOINT mousePos = DXMOUSE()->GetPos();
...
void C_DX9_HORIZONTAL_SLIDER_BAR::KeyEvent( _KEY sKey )
{
	if( !sKey.cKey )
	{
...
int C_DX9_HORIZONTAL_SLIDER_BAR::GetMinValue()
{
	return m_iMinValue;
}

int C_DX9_HORIZONTAL_SLIDER_BAR::GetMaxValue()
{
	return m_iMaxValue;
}

int C_DX9_HORIZONTAL_SLIDER_BAR::GetValue()
{
	return m_iValue;
}

int C_DX9_HORIZONTAL_SLIDER_BAR::GetStep()
{
	return m_iStep;
}

void C_DX9_HORIZONTAL_SLIDER_BAR::SetMinValue( int iMinValue )
{
	m_iMinValue = iMinValue;
}

void C_DX9_HORIZONTAL_SLIDER_BAR::SetMaxValue( int iMaxValue )
{
	m_iMaxValue = iMaxValue;
}

void C_DX9_HORIZONTAL_SLIDER_BAR::SetValue( int iValue )
{
...
void C_DX9_HORIZONTAL_SLIDER_BAR::SetStep( int iStep )
{
	m_iStep = iStep;
}

BOOL C_DX9_HORIZONTAL_SLIDER_BAR::GetDragged()
{
	return m_bDragged;
}

void C_DX9_HORIZONTAL_SLIDER_BAR::SetDragged( BOOL bDragged )
{
	m_bDragged = bDragged;
...
void C_DX9_HORIZONTAL_SLIDER_BAR::SetShowString( BOOL bShow )
{
	m_bShowString = bShow;
}

BOOL C_DX9_HORIZONTAL_SLIDER_BAR::GetShowString()
{
	return m_bShowString;
}

void C_DX9_HORIZONTAL_SLIDER_BAR::UpdateObject( int iIndex )
{
...
std::_tstring C_SET_MIN_VALUE::Calculate(C_DX9_OBJECT * pElement, LPCTSTR)
{
	std::_tstringstream sStream;

	if( pElement )
	{
		C_DX9_HORIZONTAL_SLIDER_BAR * pSlider = (C_DX9_HORIZONTAL_SLIDER_BAR*)pElement;
...
std::_tstring C_SET_MAX_VALUE::Calculate( C_DX9_OBJECT * pElement, LPCTSTR )
{
	std::_tstringstream sStream;
...
std::_tstring C_SET_SLIDER_VALUE::Calculate( C_DX9_OBJECT * pElement, LPCTSTR )
{
	std::_tstringstream sStream;

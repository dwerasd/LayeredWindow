...
void C_DX9_LIST_VERTICAL_SLIDER::SetDragged( BOOL b )
{
	m_bDragged = b;
}

BOOL C_DX9_LIST_VERTICAL_SLIDER::GetDragged()
{
	return m_bDragged;
}

C_DX9_LIST_VERTICAL_SLIDER::C_DX9_LIST_VERTICAL_SLIDER( dk::DPOINT relPos, int iHeight )
{
...
void C_DX9_LIST_VERTICAL_SLIDER::Display( dk::DPOINT basePos )
{
	dk::DPOINT Pos = basePos + *GetRelPos();

	DXENGINE()->DrawOutlinedBox( Pos.GetX(), Pos.GetY(), LIST_SLIDER_WIDTH, GetHeight(), pInner->GetD3DCOLOR(), pBorder->GetD3DCOLOR() );

	pUpArrow->Display( dk::DPOINT( Pos.GetX() + 1, Pos.GetY() + 1 ), LIST_SLIDER_WIDTH - 2, LIST_SLIDER_WIDTH - 2 );

	pDownArrow->Display( dk::DPOINT( Pos.GetX() + 1, Pos.GetY() + 1 + GetHeight() - LIST_SLIDER_WIDTH ), LIST_SLIDER_WIDTH - 2, LIST_SLIDER_WIDTH - 2 );

	if( GetMaxValue() < 2 )
		return;

	pSlider->Display( dk::DPOINT( Pos.GetX() + 1, Pos.GetY() - 1 + LIST_SLIDER_WIDTH + static_cast<int>( floor( static_cast<float>( ( GetHeight() - ( ( LIST_SLIDER_WIDTH - 2 ) * 3 ) ) ) / GetMaxValue() * GetValue() ) ) ), LIST_SLIDER_WIDTH - 2, LIST_SLIDER_WIDTH - 2 );
}

void C_DX9_LIST_VERTICAL_SLIDER::Calculate(dk::DPOINT basePos)
{
	dk::DPOINT Pos = basePos + *GetRelPos();
...
void C_DX9_LIST_VERTICAL_SLIDER::MouseMove( dk::DPOINT basePos )
{
	dk::DPOINT Pos = basePos + *GetRelPos();
...
BOOL C_DX9_LIST_VERTICAL_SLIDER::KeyEvent( dk::DPOINT basePos, _KEY sKey )
{
	dk::DPOINT Pos = basePos + *GetRelPos();
...
void C_DX9_LIST_VERTICAL_SLIDER::UpdateObject( int iIndex )
{

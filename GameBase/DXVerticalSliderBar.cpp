...
C_DX9_VERTICAL_SLIDER_BAR::C_DX9_VERTICAL_SLIDER_BAR( C_TINY_XML_ELEMENT * pElement )
{
	SetDragged( FALSE );
	SetCallback( 0 );
...
void C_DX9_VERTICAL_SLIDER_BAR::Display()
{
	...
	pSlider->Display( dk::DPOINT( Pos.GetX() + 2, Pos.GetY() + GetHeight() - static_cast<int>( floor( static_cast<float>( GetHeight() ) / GetMaxValue() * GetValue() ) ) - 5 ), WINDOW_CLOSE_BUTTON_HEIGHT, 10 );
...
void C_DX9_VERTICAL_SLIDER_BAR::MouseMove(  )
{
	C_DX9_WINDOW *pWindow = (C_DX9_WINDOW *)GetParent();
	dk::DPOINT Pos = *pWindow->GetAbsolutePos() + *GetRelPos();
...
void C_DX9_VERTICAL_SLIDER_BAR::UpdateObject( int iIndex )
{

...
C_DX9_CROSS_BUTTON::C_DX9_CROSS_BUTTON(long width, long height ) : bPressed(FALSE)
{
	SetWidth(width);
	SetHeight(height);
...
C_DX9_CROSS_BUTTON::C_DX9_CROSS_BUTTON( C_TINY_XML_ELEMENT *pElement, long width, long height ) : bPressed(FALSE)
{
	SetObject(pElement);
	if ( 0 == GetWidth() )
	{
...
void C_DX9_CROSS_BUTTON::Calculate()
{
	if(!bPressed)
	{
...
void C_DX9_CROSS_BUTTON::Display()
{
	C_DX9_WINDOW *pWindow = (C_DX9_WINDOW *)GetParent();
	dk::DPOINT Pos = *pWindow->GetAbsolutePos() + *GetRelPos();
...
void C_DX9_CROSS_BUTTON::MouseMove()
{
	C_DX9_WINDOW *pWindow = (C_DX9_WINDOW *)GetParent();
	dk::DPOINT Pos = *pWindow->GetAbsolutePos() + *GetRelPos();
...
void C_DX9_CROSS_BUTTON::KeyEvent( _KEY sKey )
{
	if( !sKey.cKey )
	{
		C_DX9_WINDOW *pWindow = (C_DX9_WINDOW *)GetParent();
...
void C_DX9_CROSS_BUTTON::UpdateObject( int iIndex )
{

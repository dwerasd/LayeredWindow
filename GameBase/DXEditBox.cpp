...
C_DX9_EDIT_BOX::C_DX9_EDIT_BOX( C_TINY_XML_ELEMENT * pElement ) : bPassword(0)
{
	SetObject( pElement );
	...
	if( pszString )
	{
		C_DX9_VARIANT * pCvar = DXENGINE()->Cvars[ pszString ];
...
void C_DX9_EDIT_BOX::KeyEvent( _KEY sKey )
{
	C_DX9_WINDOW *pWindow = (C_DX9_WINDOW *)GetParent();
	if( !sKey.cKey )
	{
		if( DXMOUSE()->GetLeftButton() )
		{
			if( GetMouseOver() )
			{
				...
				std::_tstring sString( &GetString()[ GetStart() ] );
	...
	else if( sKey.bDown && this == pWindow->GetFocussedObject() && HasFocus() )
	{
		...
		case VK_END:
			{
				std::_tstring sString = GetString();
...
int C_DX9_EDIT_BOX::GetIndex()
{
	return nIndex;
}

void C_DX9_EDIT_BOX::SetIndex( int iIndex )
{
	std::_tstring sString( &GetString()[ GetStart() ] );
...
int C_DX9_EDIT_BOX::GetStart()
{
	return nStart;
}

void C_DX9_EDIT_BOX::SetStart( int iStart )
{
	nStart = iStart;
}

void C_DX9_EDIT_BOX::UpdateObject( int iIndex )
{

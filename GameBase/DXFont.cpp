...
C_DX9_FONT::C_DX9_FONT( IDirect3DDevice9 * pDevice, int iHeight, LPTSTR pszFaceName )
{
...
void C_DX9_FONT::OnLostDevice()
{
	pFont->OnLostDevice();
}

void C_DX9_FONT::OnResetDevice()
{
	pFont->OnResetDevice();
}

void C_DX9_FONT::DrawString( int iX, int iY, DWORD dwFlags, DWORD dwColor, std::string sString, int iWidth )
{
	if( iWidth )
...
void C_DX9_FONT::DrawString( int iX, int iY, DWORD dwFlags, DWORD dwColor, std::wstring sString, int iWidth )
{
	if( iWidth )
		CutString( iWidth, sString );
...
void C_DX9_FONT::DrawStringEx( RECT rcDisplay, DWORD dwFlags, DWORD dwColor, LPSTR pszString, ... )
{
...
void C_DX9_FONT::SetColor( int iIndex, dk::DCOLOR cColor )
{
...
dk::DCOLOR & C_DX9_FONT::GetColor( int iIndex )
{
	if( iIndex < 0 || iIndex > 8 )
		return m_cColors[ 0 ];
...
int C_DX9_FONT::GetStringWidth( std::string sString )
{
...
int C_DX9_FONT::GetStringWidth( std::wstring sString )
{
	for( int i = 0; i <= static_cast<int>( sString.size() ); i++ )
	{
		if( sString[i] == ' ' )
...
int C_DX9_FONT::GetStringHeight()
{
	RECT rRect = { 0 };
...
void C_DX9_FONT::CutString( int iMaxWidth, std::string & rString )
{
...
void C_DX9_FONT::CutString( int iMaxWidth, std::wstring & rString )
{
	int iIndex = 0, iLength = rString.length();

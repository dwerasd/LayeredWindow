...
C_DX9_TEXT_BOX::C_DX9_TEXT_BOX( C_TINY_XML_ELEMENT * pElement )
{
	SetObject( pElement );

	pSlider = new C_DX9_LIST_VERTICAL_SLIDER( dk::DPOINT( GetWidth() - LIST_SLIDER_WIDTH + 2, 0 ), GetHeight() );
...
void C_DX9_TEXT_BOX::KeyEvent( _KEY sKey )
{
	C_DX9_WINDOW *pWindow = (C_DX9_WINDOW *)GetParent();
	dk::DPOINT Pos = *pWindow->GetAbsolutePos() + *GetRelPos();
...
void C_DX9_TEXT_BOX::AddString( std::_tstring sString )
{
	if( !sString.length() )
		return;

	std::vector<std::_tstring> vPending;
	...
	for( std::vector<std::_tstring>::reverse_iterator iIter = vPending.rbegin(); iIter != vPending.rend(); iIter++ )
		AddString( *iIter );
}

void C_DX9_TEXT_BOX::Clear()
{
	m_vStrings.clear();
}

void C_DX9_TEXT_BOX::UpdateObject( int iIndex )
{

...
C_DX9_BUTTON::C_DX9_BUTTON( C_TINY_XML_ELEMENT *pElement, long width, long height ) : bPressed(FALSE)
{
	SetObject(pElement);
	if ( 0 == GetWidth() )
	{
...
void C_DX9_BUTTON::KeyEvent( _KEY sKey )
{
	C_DX9_WINDOW *pWindow = (C_DX9_WINDOW *)GetParent();
	if( !sKey.cKey )
	{
		...
		{
			if( GetCallback() )
			{	// 부모 윈도우가 있으면 부모 윈도우에서 처리하게 바꿔야한다.
...
void C_DX9_BUTTON::UpdateObject( int iIndex )
{

...
C_DX9_LIST_BOX::C_DX9_LIST_BOX( C_TINY_XML_ELEMENT * pElement )
{
	SetObject( pElement );
	m_iMouseOverIndex = -1;
...
void C_DX9_LIST_BOX::KeyEvent( _KEY sKey )
{
	C_DX9_WINDOW *pWindow = (C_DX9_WINDOW *)GetParent();
	dk::DPOINT Pos = *pWindow->GetAbsolutePos() + *GetRelPos();
...
void C_DX9_LIST_BOX::AddRow( std::_tstring sString )
{
	pSlider->SetMaxValue( m_vRows.size() );
	m_vRows.push_back( sString );
}

std::_tstring C_DX9_LIST_BOX::GetRow( int iIndex )
{
...
void C_DX9_LIST_BOX::Clear()
{
	m_vRows.clear();
}

void C_DX9_LIST_BOX::UpdateObject( int iIndex )
{

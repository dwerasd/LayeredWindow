...
LRESULT CALLBACK WndProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
	...
	{
		if( DXMOUSE()->Calculate( uMsg, wParam, lParam )
			|| DXKEYBD()->Calculate( uMsg, wParam, lParam )
			)
		{
			return 0;
		}
...
long C_MAIN_CLASS::Create()
{
	// 처리 클래스 함수들을 등록
	// 메인 X버튼
	DXENGINE()->Cvars[_T("$MainCloseButton")] = new C_DX_VARIANT( new C_MAIN_CLOSE_BUTTON() );
	// 종료시 확인창 YES버튼
	DXENGINE()->Cvars[_T("$ExitWindow_button1")] = new C_DX_VARIANT( new C_EXIT_BUTTON1() );
	// 종료시확인창 NO 버튼
	DXENGINE()->Cvars[_T("$ExitWindow_button2")] = new C_DX_VARIANT( new C_EXIT_BUTTON2() );
	// 로그인 아이디 입력칸
	DXENGINE()->Cvars[_T("$LoginWindow_editbox1")] = new C_DX_VARIANT( new C_LOGIN_EDITBOX1() );
	// 로그인 패스워드 입력칸
	DXENGINE()->Cvars[_T("$LoginWindow_editbox2")] = new C_DX_VARIANT( new C_LOGIN_EDITBOX2() );
	// 로그인 확인 버튼
	DXENGINE()->Cvars[_T("$LoginWindow_button1")] = new C_DX_VARIANT( new C_LOGIN_BUTTON1() );
	// 로그인 취소 버튼
	DXENGINE()->Cvars[_T("$LoginWindow_button2")] = new C_DX_VARIANT( new C_LOGIN_BUTTON2() );

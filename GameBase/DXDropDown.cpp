...
C_DX9_DROP_DOWN::C_DX9_DROP_DOWN( C_TINY_XML_ELEMENT * pElement )
{
	SetObject( pElement );
...
void C_DX9_DROP_DOWN::Display()
{
	...
	if( pState )
	{
		...
		pButton->Display( dk::DPOINT( Pos.GetX() + GetWidth() - 19, Pos.GetY() + 1 ), 18, 18 );
	}
}

void C_DX9_DROP_DOWN::MouseMove(  )
{
	C_DX9_WINDOW *pWindow = (C_DX9_WINDOW *)GetParent();
	dk::DPOINT Pos = *pWindow->GetAbsolutePos() + *GetRelPos(), mPos = DXMOUSE()->GetPos();
...
void C_DX9_DROP_DOWN::KeyEvent( _KEY sKey )
{
	if( !sKey.cKey )
	{
...
void C_DX9_DROP_DOWN::AddObject( std::_tstring sElem, std::_tstring sValue )
{
	m_vEntrys.push_back( SEntry( sElem, sValue ) );
}

std::_tstring C_DX9_DROP_DOWN::GetValue()
{
	return m_vEntrys[ m_iSelected ].m_sValue;
}

void C_DX9_DROP_DOWN::UpdateObject( int iIndex )
{
